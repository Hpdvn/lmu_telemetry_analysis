...
class SimInfo:
    ...
    def close(self):
      ...
      try:
        self._rf2_tele.close() if self._rf2_tele else None
...
if __name__ == '__main__':
    ...
    print('Gear: %d, Clutch position: %d' % (gear, clutch))
    ...
    while True : 
        ...
        for vehicle in vehicles:
            # Check if this is the player's vehicle using mIsPlayer field
            if vehicle.mIsPlayer == 1:
                print("Found player vehicle:")
                print(f"  ID: {vehicle.mID}")
                print(f"  Vehicle: {Cbytestring2Python(vehicle.mVehicleName)}")
                print(f"  Position: {vehicle.mPlace}")
                print(f"  mDriverName: {Cbytestring2Python(vehicle.mDriverName)}")

                # fetch telemetry data for the player vehicle
                for telemetry in info.Rf2Tele.mVehicles:
                    if telemetry.mID == vehicle.mID:
                        print(f"  mFilteredBrake: {telemetry.mFilteredBrake}")
                        print(f"  mFilteredThrottle: {telemetry.mFilteredThrottle}")
                        break
                break
        else:
            print("No player vehicle found yet...")
